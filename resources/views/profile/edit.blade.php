<x-app-layout>
    ...
    <div class="font-sans text-gray-900 antialiased">
        ...
        <form method="POST" action="{{ route('profile.update', $user) }}" enctype="multipart/form-data">
            ...
            <!-- icon -->
            <div class="mt-4">
                <x-input-label for="icon" :value="__('プロフィール画像（任意・1MBまで）')" />
                <div class="rounded-full w-36">
                    <img src="{{asset('storage/icon/'.($user->icon??'user_default.jpg'))}}">
                </div>

                <x-text-input id="icon" class="block mt-1 w-full rounded-none" type="file" name="icon" :value="old('icon')" />
                ...
                <tr class="bg-white text-center">
                    ...
                    <td class="p-3">
                        <form method="post" action="{{route('role.attach', $user)}}">
                            ...
                            {{-- 「ユーザーの役割に、特定の役割が含まれているかどうか」を判定し、含まれている場合には、
                                ボタンの背景をグレーにし、さらにボタンを disable(無効）にする --}}
